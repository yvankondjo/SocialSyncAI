...
async def get_analytics_overview(
    ...
    social_accounts = (
        db.table("social_accounts").select("id").eq("user_id", user_id).execute().data
    )
    account_ids = [acc["id"] for acc in social_accounts]
    ...
    if decisions_result.data:
        ...
        ai_stats["avg_confidence"] = (
            round(total_confidence / len(decisions_result.data), 2)
            if decisions_result.data
            else 0.0
        )
    ...
    try:
        ...
        logger.warning(f"Could not fetch escalations: {e}")
        total_escalations = 0
    ...
    moderation_flags = (
        ...
        .select("id", count="exact")
        .eq("user_id", user_id)
        .like("matched_rule", "openai_moderation%")
        .gte("created_at", start_date.isoformat())
        .execute()
        .count
        or 0
...
async def get_ai_metrics(
    ...
    for decision in decisions:
        ...
        confidence_timeline[date_str]["confidences"].append(
...
async def get_topics(
    ...
    topics = (
        db.table("topic_analysis")
        .select("*")
        .eq("user_id", user_id)
        .gte("analysis_date", start_date.date().isoformat())
        .execute()
        .data
    )
    ...
    for topic in topics:
        ...
        topic_aggregates[topic_id]["sample_messages"].extend(
            topic.get("sample_messages", [])[:3]
        )

    top_topics = sorted(
        topic_aggregates.values(), key=lambda x: x["message_count"], reverse=True
    )[:10]
...
async def get_posts_comments_timeline(
    ...
    posts = (
        ...
        .select("publish_at")
        .eq("user_id", user_id)
        .gte("publish_at", start_date.isoformat())
        .execute()
        .data
    )
    ...
    try:
        ...
        monitored_post_ids = [post["id"] for post in monitored_posts]

        if monitored_post_ids:
            comments = (
                db.table("comments")
                .select("created_at")
                .in_("monitored_post_id", monitored_post_ids)
                .gte("created_at", start_date.isoformat())
                .execute()
                .data
            )
