...
def verify_instagram_webhook_signature(
    ...
    if not signature:
        logger.warning("Signature header is empty")
        return False
    ...
    if not is_valid:
        logger.debug(
            f"Signature mismatch - Expected: {expected_signature[:20]}..., "
            f"Received: {received_signature[:20]}... (first 20 chars)"
        )
...
async def instagram_webhook_handler(request: Request):
    ...
    try:
        ...
        #     )
        # if not signature:
        #     logger.warning("Missing X-Hub-Signature-256 header in Instagram webhook")
        #     raise HTTPException(status_code=403, detail="Missing signature header")
        # if not verify_instagram_webhook_signature(payload, signature, webhook_secret):
        #     logger.warning(
        #         f"Invalid Instagram webhook signature - check META_APP_SECRET in Meta for Developers. "
        #         f"Received signature: {signature[:20]}... (truncated)"
        #     )
    ...
    try:
        if "message" in message_event:
            ...
            if message.get("is_echo", False):
                logger.info(
                    f"Ignoring echo message (sent by Instagram Business Account): {message.get('text', '')[:50] if message.get('text') else 'media message'}"
                )
                return
