...
PLATFORMS = {
    ...
    "messenger": {
        ...
        "get_pages": social_auth_service.get_messenger_pages,
    },
}
...
async def handle_oauth_callback(
    ...
    try:
        ...
        if platform == "messenger":
            logger.info("📥 Messenger callback - Récupération des Pages Facebook...")
            access_token = token_data["access_token"]
            
            logger.info("🔍 Récupération des Pages Messenger...")
            pages = await platform_config["get_pages"](access_token)
            if not pages:
                logger.error("❌ Aucune Page Messenger trouvée")
                raise HTTPException(status_code=400, detail="No Facebook Page with Messenger permissions found for this user.")
            
            logger.info(f"✅ {len(pages)} Page(s) trouvée(s)")
            
            # Enregistrer chaque page comme compte social séparé
            saved_pages = 0
            failed_pages = 0
            ...
            for page in pages:
                try:
                    ...
                    if not page_id or not page_name or not page_token:
                        logger.warning(f"⚠️ Page Messenger ignorée car données manquantes: ID={page_id}, Name={page_name}, Token={'présent' if page_token else 'manquant'}")
                        failed_pages += 1
                        continue

                    logger.info(f"💾 Sauvegarde Page Messenger: {page_name} ({page_id})")
                    ...
                    logger.error(f"❌ Erreur lors de la sauvegarde de la page {page.get('name', 'Unknown')} (ID: {page.get('id', 'Unknown')}): {str(e)}")
                    failed_pages += 1
                    continue
            
            logger.info(f"✅ {saved_pages} Page(s) Messenger enregistrée(s) avec succès, {failed_pages} échouée(s) pour user {user_id}")

            if saved_pages == 0:
                logger.error("❌ Aucune page Messenger n'a pu être sauvegardée")
                raise HTTPException(status_code=500, detail="Failed to save any Messenger pages despite successful authorization.")
