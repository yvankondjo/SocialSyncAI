...
async def get_subscription_plans(db: Client = Depends(get_db)):
    """Retrieve the complete list of plans (product + prices)."""
    try:
...
async def get_my_subscription(
    ...
    """Retrieve the current subscription of the logged-in user."""
    try:
...
async def get_public_pricing(db: Client = Depends(get_db)):
    try:
        ...
        for product in result.data:
            ...
            if isinstance(features, str):
                try:
                    ...
                    features = {"text": True, "images": True, "audio": False}

            # Forcer l'accès aux images pour tous les plans
        ...
        logger.error(
            f"Erreur récupération pricing public: {error_type}: {error_message}",
            exc_info=True
        )
        logger.error(f"Supabase URL configurée: {settings.SUPABASE_URL[:30]}..." if settings.SUPABASE_URL else "Supabase URL non configurée")
        
        if "connection" in error_message.lower() or "network" in error_message.lower() or "resolve" in error_message.lower():
            raise HTTPException(
                status_code=503,
                detail="Service temporairement indisponible. Impossible de se connecter à la base de données."
            )
...
async def get_available_models(db: Client = Depends(get_db)):
    """Retrieve the list of available AI models with their costs."""
    try:
...
async def get_storage_usage(
    ...
    """Retrieve the user's storage usage."""
    try:
