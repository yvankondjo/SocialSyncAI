...
async def validate_whatsapp_credentials(credentials: WhatsAppCredentials):
    ...
    """
    try:
        async with WhatsAppService(
            credentials.access_token, credentials.phone_number_id
        ) as service:
...
async def send_text_message(request: TextMessageRequest):
    ...
    """
    try:
        user_credentials = get_user_credentials_by_platform_account(
            platform="whatsapp", account_id=request.to
        )
        ...
        service = await get_whatsapp_service(
            user_credentials["access_token"], user_credentials["phone_number_id"]
        )

        result = await service.send_text_message(
            to=request.to, text=request.text, skip_validation=True
        )
...
def verify_webhook_signature(payload: bytes, signature: str, secret: str) -> bool:
    ...
    if not secret:
        logger.warning(
            "META_APP_SECRET not configured - signature not verified"
        )
        return True
...
async def webhook_verification(
    ...
    logger.warning(
        f"Webhook WhatsApp verification failed: mode={hub_mode}, token_match={hub_verify_token == verify_token}"
    )
...
async def webhook_handler(request: Request):
    ...
    try:
        ...
        # Signature verification temporarily disabled - re-enable when META_APP_SECRET is properly configured
        ...
        #     logger.warning("Invalid webhook signature - check META_APP_SECRET")
...
async def process_webhook_entry_with_user_routing(entry: dict):
    ...
    phone_number_id = (
        entry.get("changes")[0].get("value").get("metadata").get("phone_number_id")
    )
    ...
    logger.info(
        f"Webhook routed to user {user_info['user_id']} (phone: {phone_number_id})"
    )
