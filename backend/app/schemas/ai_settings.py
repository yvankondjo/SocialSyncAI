...
class AISettingsBase(BaseModel):
    ...
    ai_enabled_for_chats: bool = Field(default=True, description="Enable/disable AI for private messages (DMs, WhatsApp)")
    ...
    flagged_keywords: Optional[List[str]] = Field(default_factory=list, description="Keywords to ignore (short messages like 'ok', 'merci', etc.)")
    flagged_phrases: Optional[List[str]] = Field(default_factory=list, description="Phrases to ignore (e.g., 'ça va', 'merci beaucoup', etc.)")
    instructions: Optional[str] = Field(None, max_length=5000, description="Free-text instructions for AI (e.g. '- Avoid spam\\n- Be polite')")
    ignore_examples: Optional[List[str]] = Field(default_factory=list, description="Array of example messages to NOT respond to")

    @field_validator('doc_lang', 'flagged_keywords', 'flagged_phrases', 'ignore_examples', mode='before')
    @classmethod
...
class AISettingsUpdate(BaseModel):
    ...
    ai_control_enabled: Optional[bool] = Field(None, description="Master toggle for AI")
    ai_enabled_for_chats: Optional[bool] = Field(None, description="Enable/disable AI for private messages")
    ai_enabled_for_comments: Optional[bool] = Field(None, description="Enable/disable AI for public comments")
    doc_lang: Optional[List[str]] = Field(None, description="Liste des langues des documents de l'utilisateur")
    flagged_keywords: Optional[List[str]] = Field(None, description="Keywords to ignore")
    flagged_phrases: Optional[List[str]] = Field(None, description="Phrases to ignore")
    instructions: Optional[str] = Field(None, max_length=5000, description="Free-text instructions for AI")
    ignore_examples: Optional[List[str]] = Field(None, description="Example messages to NOT respond to")
