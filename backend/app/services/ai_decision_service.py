...
Evaluates whether the AI should respond to a message based on user rules
and the OpenAI Moderation API.
"""
import os
import logging
...
class AIDecisionService:
    """Service to evaluate whether the AI should respond to a message."""

    def __init__(self, user_id: str, db: Client = None):
        ...
        if api_key:
            ...
            logger.warning(
                "[MODERATION] OPENAI_API_KEY missing - disabling OpenAI guardrails"
            )
    ...
    def check_message(
        ...
        Args:
            ...
            context_type: Context type - "chat" (DMs) or "comment" (public posts)
            message_content: Optional rich message content (for future use with images/attachments)
    ...
    def _check_openai_moderation(self, text: str) -> Dict[str, Any]:
        ...
        if not self.moderation_enabled or not self.openai_client:
            logger.debug("[MODERATION] OpenAI moderation disabled (missing key)")
            return {"flagged": False, "error": "moderation_disabled"}
    ...
    def _get_user_rules(self) -> Optional[Dict[str, Any]]:
        """Retrieve AI settings for the user from DB"""
        try:
            result = self.db.table("ai_settings") \
                .select("*") \
