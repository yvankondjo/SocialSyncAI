...
class AnalyticsService:
    def __init__(self):
        self.platform_handlers = {'twitter': self._fetch_twitter_stats, 'instagram': self._fetch_instagram_stats, 'facebook': self._fetch_facebook_stats, 'linkedin': self._fetch_linkedin_stats, 'youtube': self._fetch_youtube_stats, 'tiktok': self._fetch_tiktok_stats, 'whatsapp': self._fetch_whatsapp_stats}
    ...
    async def sync_content_analytics(self, db: Client, content_id: str) -> dict:
        ...
        try:
            ...
            db.table('analytics').upsert({'content_id': content_id, 'platform': social_account['platform'], **self._format_stats(stats)}).execute()
            db.table('analytics_history').insert({'content_id': content_id, 'platform': social_account['platform'], 'user_id': social_account['user_id'], **self._format_stats(stats)}).execute()
    ...
    def _format_stats(self, stats: dict) -> dict:
        """Helper to format stats for DB insertion."""  # inserted
        return {'likes': stats.get('likes', 0), 'shares': stats.get('shares', 0), 'comments': stats.get('comments', 0), 'impressions': stats.get('impressions', 0), 'reach': stats.get('reach', 0), 'clicks': stats.get('clicks', 0), 'conversions': stats.get('conversions', 0), 'engagement_rate': stats.get('engagement_rate', 0), 'raw_metrics': stats.get('raw_metrics', {}), 'recorded_at': datetime.now(timezone.utc).isoformat()}
    ...
    async def _fetch_twitter_stats(self, social_account: Dict, content: Dict) -> Dict:
        ...
        try:
            ...
            tweet_id = content.get('account_id')
    ...
    def _calculate_engagement_rate(self, metrics: Dict) -> float:
        ...
        total_engagements = metrics.get('like_count', 0) + metrics.get('retweet_count', 0) + metrics.get('reply_count', 0)
        impressions = metrics.get('impression_count', 0)
        if impressions > 0:
