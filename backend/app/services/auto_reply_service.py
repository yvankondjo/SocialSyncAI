...
class AutoReplyService:
    ...
    def get_user_auto_reply_settings(self, user_id: str, platform: str = 'whatsapp') -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return self.create_default_settings(user_id, platform)
        except Exception as e:
            logger.error(f'Erreur lors de la récupération des paramètres auto-reply: {e}')
            return None

    def create_default_settings(self, user_id: str, platform: str = 'whatsapp') -> Optional[Dict[str, Any]]:
    ...
    def update_auto_reply_settings(self, user_id: str, is_enabled: bool, platform: str = 'whatsapp', additional_settings: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            if not response.data:
                self.create_default_settings(user_id, platform)
                response = self.supabase.table('auto_reply_settings').update(update_data).eq('user_id', user_id).eq('platform', platform).execute()
    ...
    def is_auto_reply_enabled(self, user_id: str, platform: str = 'whatsapp') -> bool:
        """Vérifie si les réponses automatiques sont activées pour un utilisateur"""
        try:
            settings = self.get_user_auto_reply_settings(user_id, platform)
    ...
    def get_auto_reply_message(self, user_id: str, platform: str = 'whatsapp') -> Optional[str]:
    ...
    def set_auto_reply_message(self, user_id: str, message: str, platform: str = 'whatsapp') -> bool:
        """Définit le message de réponse automatique"""
        try:
            additional_settings = {'message': message}
            return self.update_auto_reply_settings(user_id, True, platform, additional_settings)
        except Exception as e:
