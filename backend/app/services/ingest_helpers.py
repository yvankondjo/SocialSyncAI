...
def detect_language(text: str) -> tuple[str, str]:
    try:
        ...
        d = detect(sample)
        m = {
            "fr": ("french", "pg_catalog.french"),
            "en": ("english", "pg_catalog.english"),
            "es": ("spanish", "pg_catalog.spanish"),
            "de": ("german", "pg_catalog.german"),
            "it": ("italian", "pg_catalog.italian"),
            "pt": ("portuguese", "pg_catalog.portuguese"),
        }
...
async def add_context_to_chunks(
    ...
    if len(document_text) > 700_000:
        document_text = document_text[:700_000]
    client = AsyncOpenAI(
    ...
    async def one(c: Tuple[str,int,int]) -> Tuple[str,int,int]:
        ...
        messages = [
            {"role":"system","content":(
                "You are a retrieval assistant. Given a chunk from the user, "
                "return a concise 1-4 sentence context label that situates the chunk within the cached document. "
                "Be specific, no fluff. YOU MUST USE THE SAME LANGUAGE AS THE DOCUMENT."
            )},
...
YOU MUST USE THE SAME LANGUAGE AS THE DOCUMENT."""}]},
        ...
        async with sem:
            r = await asyncio.wait_for(
                client.chat.completions.create(model=model, messages=messages, temperature=0.75, max_tokens=256),
                timeout=timeout_s
            )
        ctx = (r.choices[0].message.content or "").strip()
        return (f"{ctx} {chunk_text}".strip(), c[1], c[2])

    tasks = [one(c) for c in chunks]
