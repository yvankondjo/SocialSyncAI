...
class InstagramService:
    ...
    async def send_direct_message(self, recipient_ig_id: str, text: str) -> Dict[str, Any]:
        logger.info(f'📤 [INSTAGRAM] send_direct_message called: recipient={recipient_ig_id}, text_length={len(text)}, page_id={self.page_id}')
        url = f'/{self.page_id}/messages'
        payload = {'recipient': {'id': recipient_ig_id}, 'message': {'text': text}, 'access_token': self.access_token}
        logger.info(f'📤 [INSTAGRAM] Sending DM to {recipient_ig_id}: {text[:100]}...')
        try:
            ...
            logger.error(f'❌ [INSTAGRAM] Error sending DM: {e}', exc_info=True)
