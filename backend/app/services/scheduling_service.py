...
from app.schemas.scheduling import (
    PlatformPreview, PlatformType, ScheduledPost, PostStatus
)
...
class SchedulingService:
    ...
    def _generate_reddit_preview(self, content: str, media_urls: Optional[List[str]]) -> PlatformPreview:
        ...
        # Extraire un titre potentiel (première ligne ou 100 premiers caractères)
    ...
    async def schedule_post(
        ...
        try:
            ...
            response = self.supabase.table('scheduled_posts').insert(post_data).execute()
            
            if response.data:
                return ScheduledPost(**response.data[0])
            
            raise ValueError("Échec de la création du post planifié")
    ...
    async def get_calendar_posts(
        ...
        try:
            ...
            if platforms:
                ...
                query = query.overlaps('platforms', platform_values)
            
            # Ordonner par date de planification
            ...
            for row in response.data:
                posts.append(ScheduledPost(**row))
    ...
    async def cancel_scheduled_post(self, post_id: str, user_id: str) -> bool:
        ...
        try:
            response = self.supabase.table('scheduled_posts').update({
