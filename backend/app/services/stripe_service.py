...
class StripeService:
    """
    Stripe service for the hybrid architecture.
    ...
    async def get_or_create_customer(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error getting/creating Stripe customer {user_id}: {e}")
            raise HTTPException(status_code=500, detail="Error managing Stripe customer")
    ...
    async def create_checkout_session(self, user_id: str, price_id: str,
        ...
        try:
            ...
            logger.error(f"Error creating checkout session {user_id}: {e}")
    ...
    async def handle_webhook(self, payload: bytes, signature: str) -> Dict[str, Any]:
        """
        Process a Stripe webhook by validating the signature and parsing the payload.
        """
        if not self.enabled:
            return {'status': 'disabled', 'message': 'Stripe service disabled'}

        try:
            # Validate and parse the Stripe webhook
    ...
    async def _process_webhook_event(self, event: Dict[str, Any]) -> Dict[str, Any]:
        """
        Process a Stripe webhook with idempotency.

        Standard event handling:
        - product.* : product sync
        - price.* : price sync
        - customer.subscription.* : subscription sync
        - invoice.* : payment/renewal handling
        """
        if not self.enabled:
            return {'status': 'disabled', 'message': 'Stripe service disabled'}
        ...
        try:
            ...
            # Try-insert (unique) for idempotency. On conflict => already processed.
            try:
                self.db.table('webhook_events').insert({
                    'stripe_event_id': event_id,
                    'event_type': event_type,
                    'source': 'stripe',
                    'payload': event,            # can be truncated if too large
                    'processed_at': datetime.now(timezone.utc).isoformat()
                }).execute()
            except Exception as e:
                # Unique constraint error -> already processed
                logger.info(f"Event already processed (unique conflict): {event_id}")
    ...
    async def _sync_product(self, product: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Product synced: {product['id']} - {product['name']}")
    ...
    async def _archive_product(self, product: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Product archived: {product['id']}")
            return {'archived': True, 'product_id': product['id']}
                
        except Exception as e:
            logger.error(f"Error archiving product {product['id']}: {e}")
            raise
    ...
    async def _sync_price(self, price: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Price synced: {price['id']} - {price['unit_amount']/100}€")
            return {'synced': True, 'price_id': price['id']}

        except Exception as e:
            logger.error(f"Error syncing price {price['id']}: {e}")
            raise

    async def _archive_price(self, price: Dict[str, Any]) -> Dict[str, Any]:
        """Archive a deleted price."""
        try:
            ...
            logger.info(f"Price archived: {price['id']}")
            return {'archived': True, 'price_id': price['id']}

        except Exception as e:
            logger.error(f"Error archiving price {price['id']}: {e}")
            raise

    async def _handle_subscription_created(self, subscription: Dict[str, Any]) -> Dict[str, Any]:
        """Handle subscription creation, including trial credits management."""
        try:
            user_id = subscription.get('metadata', {}).get('user_id')

            if not user_id:
                # Try to find it via customer
            ...
            if not user_id:
                logger.warning(f"User ID not found for subscription {subscription['id']}")
            ...
            # CREDIT MANAGEMENT BY STATUS
            if subscription['status'] == 'trialing':
                # APPLY TRIAL CREDITS (100 credits)
                await self._apply_trial_credits(user_id, subscription['id'])
            elif subscription['status'] == 'active':
                # ACTIVE SUBSCRIPTION (no trial) - apply plan credits
            ...
            logger.info(f"Subscription created: {subscription['id']} for user {user_id} - status: {subscription['status']}")
            return {
    ...
    def _calculate_next_reset(self) -> str:
        """Calculate the date of the next monthly reset (beginning of the next month)."""
        from datetime import datetime, timezone
