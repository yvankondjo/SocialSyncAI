...
def poll_post_comments():
    ...
    try:
        ...
        result = db.table("monitored_posts") \
            .select("*, social_accounts!inner(platform, access_token, account_id, username)") \
...
def process_comment(self, comment_id: str):
    ...
    try:
        ...
        if decision == AIDecision.ESCALATE:
            ...
            from app.deps.credit_tracker import get_or_create_credit_tracker
            ...
            # Format system prompt with doc_lang
            ...
            # Create credit tracker for enterprise features
            credits_service = CreditsService(db)
            credit_tracker = asyncio.run(
                get_or_create_credit_tracker(user_id, credits_service)
            )

            # Create RAG agent with all enterprise features
            ...
            # Finalize credits after response generation
            asyncio.run(
                credit_tracker.finalize_batch(conversation_id=f"comment:{comment_id}")
            )
