...
export default function ComparePage() {
  ...
  const handleSendMessage = async () => {
    ...
    // Prepare messages history for both conversations
    const leftMessages = [...conversations.left.messages, userMessage].filter(
      (msg) => msg.role === "user" || msg.role === "assistant"
    )
    const rightMessages = [...conversations.right.messages, userMessage].filter(
      (msg) => msg.role === "user" || msg.role === "assistant"
    )
    ...
    const rightTestRequest = {
      ...
      messages: rightMessages.map((msg) => ({
        role: msg.role,
        content: msg.content,
      })),
      settings: {
