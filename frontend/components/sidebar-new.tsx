...
import {
  ...
  ChevronDown,
  Plus,
  Plus as LogOut, // Replaced LogOut
  User,
  Plus as SettingsIcon, // Replaced Settings
  Plus as Crown, // Replaced Crown
  Plus as Sparkles, // Replaced Sparkles
  Plus as HardDrive, // Replaced HardDrive
} from "lucide-react"
      ...
      <nav className="flex-1 p-4 space-y-1">
        {navigation.map((item) => {
          ...
          if (item.type === "section") {
            const isSectionActive = pathname ? pathname.startsWith(item.href) : false
                ...
                {isExpanded && !isCollapsed && item.children && (
                  <div className="ml-6 mt-1 space-y-1">
                    {item.children.map((child) => {
                      const isChildActive = pathname ? (pathname === child.href || pathname.startsWith(child.href + "/")) : false
